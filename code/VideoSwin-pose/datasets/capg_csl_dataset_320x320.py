...
def load_rgb_frames(frame_paths, sampler, kpts_2d, img_norm):
    ...
    for i in list(indexes):
        ...
        right_hand_img = cv2.imread(frame_paths[i].replace('rgb-320x320', 'right-hand-224x224'))[:, :, [2, 1, 0]]
        left_hand_img = cv2.imread(frame_paths[i].replace('rgb-320x320', 'left-hand-224x224'))[:, :, [2, 1, 0]]
        face_hand_img = cv2.imread(frame_paths[i].replace('rgb-320x320', 'face-224x224'))[:, :, [2, 1, 0]]
...
def make_dataset(split, root, num_classes, 
    ...
    i = 0
    for path in sorted(glob(f"{vid_root}/rgb-320x320/*/*/*/camera_*")):
        if path[-8:-1] != 'camera_':
            continue
