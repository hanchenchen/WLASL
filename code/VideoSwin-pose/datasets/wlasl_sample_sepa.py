...
def pose_filtering(video_path):
    ...
    cur_pose = json.load(open(cur_img.replace('images', 'images-pose').replace('.png', '_keypoints.json'), 'r'))['people'][0]['pose_keypoints_2d']
...
def get_pose(frame_paths):
    ...
    pose_path = frame_paths[0].replace('images', 'images-pose').replace('.png', '_keypoints.json')
    ...
    for poses_idx in range(len(frame_paths)):
        pose_path = frame_paths[poses_idx].replace('images', 'images-pose').replace('.png', '_keypoints.json')
        pose = json.load(open(pose_path, 'r'))['people'][0]
...
def load_rgb_frames(frame_paths, sampler,pose_list):
    frames = []
    right_hand = []
    left_hand = []
    face = []
    ...
    for frames_idx, right_hand_idx, left_hand_idx, face_idx, poses_idx in zip(frames_indexes, right_hand_indexes, left_hand_indexes, face_indexes, poses_indexes):
        ...
        right_hand_path = frame_paths[right_hand_idx].replace('images', 'right-hand-224x224')
        right_hand_img = cv2.imread(right_hand_path)[:, :, [2, 1, 0]]

        left_hand_path = frame_paths[left_hand_idx].replace('images', 'left-hand-224x224')
        left_hand_img = cv2.imread(left_hand_path)[:, :, [2, 1, 0]]

        face_path = frame_paths[face_idx].replace('images', 'face-224x224')
...
def make_dataset(split, root, num_classes, 
    ...
    for vid in data.keys():
        ...
        frame_paths = sorted(glob(f"{video_path}/*"))

        valid_frame_paths = []
        for p in  frame_paths:
            if os.path.exists(p.replace('images', 'right-hand-224x224')):
                valid_frame_paths.append(p)
        num_frames = len(valid_frame_paths)
        ...
        valid_pose_list = get_pose(valid_frame_paths)
        dataset.append((label, video_path, valid_frame_paths, valid_pose_list))
        i += 1
...
class CAPG_CSL(data_utl.Dataset):
    ...
    def __getitem__(self, index):
        ...
        imgs, right_hand, left_hand, face, poses = load_rgb_frames(frame_paths, self.sample_frame, pose_list)
