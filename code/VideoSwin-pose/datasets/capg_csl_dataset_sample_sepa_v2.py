...
def load_rgb_frames(frame_paths, sampler, img_norm, img_index_map, index_view_img_map, split, pose_list):
    ...
    indexes = indexes.reshape(-1, 5)
    frames_indexes = list(indexes[:,0])
    right_hand_indexes = list(indexes[:,1])
    left_hand_indexes = list(indexes[:,2])
    face_indexes = list(indexes[:,3])
    poses_indexes = list(indexes[:,4])
...
class CAPG_CSL(data_utl.Dataset):
    ...
    view_list=['camera_0', 'camera_1', 'camera_2', 'camera_3']):
        ...
        self.sample_frame = SampleFrames(clip_len=1, num_clips=self.total_frames*5, test_mode=self.split!='train')
