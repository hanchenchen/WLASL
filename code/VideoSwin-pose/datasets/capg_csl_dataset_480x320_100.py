...
def load_rgb_frames(frame_paths, sampler, img_norm):
    frames = []
    right_hand = []
    left_hand = []
    face = []
    ...
    pose = json.load(open(frame_paths[0].replace('rgb-480x320', 'openpose-res').replace('.jpg', '_keypoints.json'), 'r'))['people'][0]
    ...
    for i in list(indexes):
        ...
        pose = json.load(open(frame_paths[i].replace('rgb-480x320', 'openpose-res').replace('.jpg', '_keypoints.json'), 'r'))['people'][0]
...
def record_view_map(video_path, img_index_map, index_view_img_map):
    ...
    for i in range(len(frame_paths)):
        ...
        index_view_img_map[record_time][i][view] = frame_paths
    return img_index_map, index_view_img_map

def view_aug(frame_path, img_index_map, index_view_img_map):
    label, signer, record_time, view, img_name = frame_path.split('/')[-5:]
    img_idx = img_index_map[img_name]
    view = random.choice(['camera_0', 'camera_1', 'camera_2', 'camera_3'])
...
class CAPG_CSL(data_utl.Dataset):
    ...
    def __getitem__(self, index):
        ...
        imgs, right_hand, left_hand, face, poses = load_rgb_frames(frame_paths, self.sample_frame, self.img_norm)
