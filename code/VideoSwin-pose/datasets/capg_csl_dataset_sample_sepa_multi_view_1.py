...
class CAPG_CSL(data_utl.Dataset):
    ...
    view_list=['camera_0', 'camera_1', 'camera_2', 'camera_3']):
        ...
        self.sample_frame = SampleFrames(clip_len=1, num_clips=self.total_frames, test_mode=self.split!='train')
    ...
    def __getitem__(self, index):
        if self.split == 'train':
            ...
            for i in range(1):
                img, lab, _, pose, right_hand, left_hand, face = self.getitem(index)
                img_list.append(img)
                lab_list.append(lab)
            ...
            lab = torch.stack(lab_list, dim=0)
            pose = torch.stack(pose_list, dim=0)
            right_hand = torch.stack(right_hand_list, dim=0)
            left_hand = torch.stack(left_hand_list, dim=0)
            face = torch.stack(face_list, dim=0)
            return img, lab, index, pose, right_hand, left_hand, face
