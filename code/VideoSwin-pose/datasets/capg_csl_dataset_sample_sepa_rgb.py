...
def pose_filtering(video_path):
    ...
    cur_pose = json.load(open(cur_img.replace('rgb-1280x1280', 'openpose-res').replace('.jpg', '_keypoints.json'), 'r'))['people'][0]['pose_keypoints_2d']
...
def get_pose(frame_paths):
    ...
    pose_path = frame_paths[0].replace('rgb-1280x1280', 'openpose-res').replace('.jpg', '_keypoints.json')
    ...
    for poses_idx in range(len(frame_paths)):
        pose_path = frame_paths[poses_idx].replace('rgb-1280x1280', 'openpose-res').replace('.jpg', '_keypoints.json')
        pose = json.load(open(pose_path, 'r'))['people'][0]
...
def make_dataset(split, root, num_classes, 
    ...
    for vid_root in vid_root_list:
        for path in sorted(glob(f"{vid_root}/rgb-1280x1280/*/*/*/camera_*")):
            if path[-8:-1] != 'camera_':
                continue
