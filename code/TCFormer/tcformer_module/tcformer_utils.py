...
def token2map(token_dict):
    ...
    idx_hw = get_grid_index([H_init, W_init], [H, W], device=device)[None, :].expand(
        B, -1
    )
...
def map2token(feature_map, token_dict):
    ...
    if N_init < N * H * W:
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            all_weight = (
                A @ torch.ones([B * H * W, 1], device=device, dtype=torch.float32)
                + 1e-6
            )
...
def token_downup(target_dict, source_dict):
    ...
    if N_init < T * S:
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            # normalize the matrix
            all_weight = (
                A.type(torch.float32) @ x_s.new_ones(B * S, 1).type(torch.float32)
                + 1e-6
            )
        ...
        coor = torch.stack([idx_batch, idx_token_t, idx_token_s], dim=0).reshape(
            3, B * N_init
        )
...
def vis_tokens(img, token_dict, edge_color=[1.0, 1.0, 1.0], edge_width=1):
    ...
    for i in range(edge_width):
        ...
        idx_map = (
            idx_map * (~edge_map)
            + torch.rand(idx_map.shape, device=device, dtype=dtype) * edge_map
        )
...
def get_token_density_map(token_dict):
    ...
    token_density = (
        1 / torch.sparse.FloatTensor(coor, tmp, torch.Size([B, N])).to_dense()
    )
    tmp_dict = token_dict.copy()
